...
describe('useHashState', () => {
  ...
  test('rejects JSON not matching initialState keys', () => {
    ...
    const { result } = renderHook(() => useHashState(initialState, { validateKeysAndTypes: true }));
    const { state } = result.current;
    expect(state).toEqual(initialState);
    expect(console.warn).toHaveBeenNthCalledWith(1, 'Object in URL hash is invalid, ignoring');
  ...
  test('rejects JSON not matching custom validator', () => {
    ...
    const validator: JSONValidator<State> = (obj): boolean => {
    ...
    location.hash = `#${encodeURIComponent(JSON.stringify(invalidState))}`;
    const { result } = renderHook(() => useHashState<State>(initialState, { customValidator: validator }));
    const { state } = result.current;
    expect(state).toEqual(initialState);
    expect(console.warn).toHaveBeenNthCalledWith(1, 'Object in URL hash is invalid, ignoring');
    expect(console.warn).toHaveBeenNthCalledWith(2, invalidState);
  ...
  test('uses pushHistory if desired', () => {
    ...
    const { result } = renderHook(() => useHashState<State>(initialState, { usePushState: true }));
    const { state, setStateAtKey } = result.current;
    expect(state).toEqual(initialState);
